...
import com.lin.store.presenter.SearchPresenter;
import com.lin.store.utils.RecordSQLiteOpenHelper;
import com.lin.store.view.SearchView;

/**
 * Created by lin on 2019/5/10.
 */
@Route(path = ArouterPath.ACTIVITY_STORE_SEARCH)
        ...
        etSearch.setOnKeyListener(new View.OnKeyListener() {
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_DOWN) {
                    ...
                    ARouter.getInstance().build(ArouterPath.ACTIVITY_STORE_SEARCH_RESULT).withString("key",key).navigation();
                }
                return false;
            }
        });
