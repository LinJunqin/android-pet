...
public class SearchResultPresenter extends BasePresenter<SearchResultView>{
    ...
    public void search(String key) {
        addDisposable(GoodsHttpClient.getGoodsOfSearch(key), new BaseObserver<BaseResponse>(baseView) {
            @Override
            public void onSuccess(BaseResponse response) {
                ...
                List<Goods> goods = gson.fromJson(gson.toJson(response.getData()), new TypeToken<List<Goods>>(){}.getType());
                baseView.updateList(goods);
            }
        });
    }
}
